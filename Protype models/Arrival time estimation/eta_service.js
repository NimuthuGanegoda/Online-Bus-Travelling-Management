...
// ── Configuration ────────────────────────────────────────────────────────────
const CONFIG = {
  pythonPath: "C:\\Users\\Nimuthu Ganegoda\\AppData\\Local\\Microsoft\\WindowsApps\\PythonSoftwareFoundation.Python.3.11_qbz5n2kfra8p0\\python.exe",
  scriptPath: path.resolve(__dirname, "predict_arrival.py"),
  csvPath: path.resolve(__dirname, "bus_trips_dataset.csv"),
  busRatingDbPath: path.resolve(__dirname, "Bus rating", "database.db"),
 ...
 */
function getDriverRating(driverName) {
  return new Promise((resolve, reject) => {
    const db = new sqlite3.Database(CONFIG.busRatingDbPath);

    db.all(
      "SELECT comment FROM comments WHERE driver_name = ?",
      [driverName],
      (err, rows) => {
        if (err) {
          db.close();
        ...
        if (!rows || rows.length === 0) {
          db.close();
          // Return default rating if no comments found
          return resolve(4.0);
        }
        ...
        py.on("close", (code) => {
          ...
          try {
            // Parse individual ratings and calculate average
        ...
        // Send comments to Python via stdin
